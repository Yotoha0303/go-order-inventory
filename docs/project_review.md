...
修正：增加 parsePositiveID，统一校验 id 必须为正整数。
...
已优化：订单创建和取消订单中已使用 FOR UPDATE 锁定库存行，提高库存流水 before/after 的准确性。

后续优化：可增加并发测试验证库存扣减和库存流水一致性。
...
当前项目使用 `global.DB` 和 `global.Redis` 保存基础设施资源，适合轻量单体项目快速开发。后续如果项目规模扩大，可改为依赖注入，将 DB、Redis、Logger 传入 service，提高测试和维护能力。

### 2. Service 结构

当前 service 层采用函数式写法，直接调用 dao 和 global 资源。后续可改为结构体 service，例如 `OrderService{db, redis}`，降低全局状态依赖。

### 3. 缓存降级

当前 Redis 缓存失败时不影响主业务流程，直接回退数据库。后续可接入日志和监控，记录缓存命中率和 Redis 错误率。

### 4. 订单号生成

当前订单号使用时间戳生成，并通过数据库唯一索引兜底。后续可替换为雪花 ID 或独立订单号生成器。
